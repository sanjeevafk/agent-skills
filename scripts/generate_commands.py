...
def yaml_string(value: str) -> str:
    """Return a safely quoted YAML string for generated frontmatter."""
...
def generate_commands():
    ...
    # Replace only a directory explicitly marked as generated.
    if COMMANDS_DIR.exists():
        marker = COMMANDS_DIR / '.generated-by-agent-skills'
        if not marker.exists():
            raise RuntimeError(
                f"Refusing to delete unmarked commands directory: {COMMANDS_DIR}"
            )
        shutil.rmtree(COMMANDS_DIR)
    COMMANDS_DIR.mkdir(parents=True, exist_ok=True)
    (COMMANDS_DIR / '.generated-by-agent-skills').write_text(
        'Generated by scripts/generate_commands.py\n', encoding='utf-8'
    )
    ...
    for name, meta in skills.items():
        ...
        desc_escaped = yaml_string(meta['description'])
